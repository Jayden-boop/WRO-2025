...
import math
import time
from gyro_test import ros_robot_controller_sdk as rrc
...
class GyroLoopDriver(Node):
    def __init__(self):
        ...
        )  # The QoS (Quality of Service) depth - this sets the size of the message queue buffer
        ...
        self.controlTimer = self.create_timer(
            0.01, self.controlLoop
        )  # controlLoop is fired every 0.01 second
    ...
    def controlLoop(self):
        ...
        if self.state.startswith("DRIVE"):
            ...
            if self.turnStarted == False:
                ...
                self.steer(
                    -1.0
                )  # counterclockwise turn (to be replaced with servo sharp turn)
            ...
            if self.angleReached(targetAngle):
                ...
                self.get_logger().info(
                    f"Reached {targetAngle}°, switching to {self.state}"
                )
